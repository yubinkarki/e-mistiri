// App logo.
import MainLogo from '../assets/svg/logo/MainLogo.svg';
import MainLogoColor from '../assets/svg/logo/MainLogoColor.svg';

// Icons for bottom tab navigation bar.
import Home from '../assets/svg/bottom_nav/Home.svg';
import Cart from '../assets/svg/bottom_nav/Cart.svg';
import Account from '../assets/svg/bottom_nav/Account.svg';
import Shop from '../assets/svg/bottom_nav/Shop.svg';

// Onboarding images.
import Onboarding1 from '../assets/svg/onboarding/Onboarding1.svg';
import Onboarding2 from '../assets/svg/onboarding/Onboarding2.svg';
import Onboarding3 from '../assets/svg/onboarding/Onboarding3.svg';

// Dashboard screen images.
import SearchIcon from '../assets/svg/dashboard/SearchIcon.svg';
import FilterButtonIcon from '../assets/svg/dashboard/FilterButtonIcon.svg';
import Bike from '../assets/svg/dashboard/Bike.svg';
import Car from '../assets/svg/dashboard/Car.svg';
import Scooter from '../assets/svg/dashboard/Scooter.svg';
import Accessories from '../assets/svg/dashboard/Accessories.svg';

// Product images.
import BrakePad from '../assets/svg/product/BrakePad.svg';
import CastrolMagnatec from '../assets/svg/product/CastrolMagnatec.svg';
import EngineFilter from '../assets/svg/product/EngineFilter.svg';

// Account screen images.
import DefaultProfile from '../assets/images/DefaultProfile.png';
import RewardPointRibbon from '../assets/svg/account/RewardPointRibbon.svg';
import AboutUs from '../assets/svg/account/AboutUs.svg';
import Logout from '../assets/svg/account/Logout.svg';
import MyInformation from '../assets/svg/account/MyInformation.svg';
import Settings from '../assets/svg/account/Settings.svg';
import Share from '../assets/svg/account/Share.svg';
import RightArrow from '../assets/svg/account/RightArrow.svg';
import ChangeLanguage from '../assets/svg/account/ChangeLanguage.svg';
import ChangePassword from '../assets/svg/account/ChangePassword.svg';
import DarkMode from '../assets/svg/account/DarkMode.svg';
import Notification from '../assets/svg/account/Notification.svg';
import Privacy from '../assets/svg/account/Privacy.svg';
import CheckMark from '../assets/svg/account/CheckMark.svg';
import FlagFrance from '../assets/svg/account/FlagFrance.svg';
import FlagEngland from '../assets/svg/account/FlagEngland.svg';

// Cart screen images.
import RegularService from '../assets/svg/cart/RegularService.svg';
import Cross from '../assets/svg/cart/Cross.svg';
import CounterMinus from '../assets/svg/cart/CounterMinus.svg';
import CounterPlus from '../assets/svg/cart/CounterPlus.svg';
import PromotionText from '../assets/svg/cart/PromotionText.svg';
import PromotionCar from '../assets/svg/cart/PromotionCar.svg';

export default {
  mainLogo: MainLogo,
  mainLogoColor: MainLogoColor,
  onboarding1: Onboarding1,
  onboarding2: Onboarding2,
  onboarding3: Onboarding3,
  searchIcon: SearchIcon,
  filterIcon: FilterButtonIcon,
  brakePad: BrakePad,
  castrolMagnatec: CastrolMagnatec,
  engineFilter: EngineFilter,
  bike: Bike,
  car: Car,
  scooter: Scooter,
  accessories: Accessories,
  homeIcon: Home,
  cartIcon: Cart,
  accountIcon: Account,
  shopIcon: Shop,
  defaultProfile: DefaultProfile,
  rewardPointRibbon: RewardPointRibbon,
  aboutUs: AboutUs,
  logout: Logout,
  myInformation: MyInformation,
  settings: Settings,
  share: Share,
  rightArrow: RightArrow,
  changeLanguage: ChangeLanguage,
  changePassword: ChangePassword,
  darkMode: DarkMode,
  notification: Notification,
  privacy: Privacy,
  checkMark: CheckMark,
  flagFrance: FlagFrance,
  flagEngland: FlagEngland,
  regularService: RegularService,
  cross: Cross,
  counterMinus: CounterMinus,
  counterPlus: CounterPlus,
  promotionText: PromotionText,
  promotionCar: PromotionCar,
};
